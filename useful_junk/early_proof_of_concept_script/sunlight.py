import numpy as np
import pandas as pd
from pytt.utils import dates, coords
...
def sza(glat, glon, datetimes, degrees = True):
    ...
    ssr = coords.sph_to_car(np.vstack((np.ones_like(sslat), 90. - sslat, sslon)), deg = True)
    gcr = coords.sph_to_car(np.vstack((np.ones_like(glat ), 90. - glat , glon )), deg = True)
...
def terminator(datetime, sza = 90, resolution = 360):
    ...
    tsph = coords.car_to_sph(r, deg = True)
