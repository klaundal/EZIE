""" script to produce figure 1 in the eletrojet inversion paper

Checked that it works 13 August 2021 (although you'll need to intall basemap...)
"""
...
scale_size = np.array([1e-3, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900]) # km
...
# using Jean's formula
n = np.round(2 * np.pi * (A + 200) / scale_size - 1/2)
...
if SQRTPOWER:
    Bvsh    = np.sqrt(Bvsh/Bvsh[-1])*500
else:
    Bvsh    = Bvsh/Bvsh[-1]*500
ax_ps.plot(2* np.pi*(A + 200) / (nn.flatten() + 1/2), amplitude_of_n(nn.flatten()), color = 'grey', label = 'Fitted and extrapolated')
...
ax_psn.set_xticks(2 * np.pi * (A + 200) / (np.array(n_tick_locations) + 1/2))
ax_psn.set_xticklabels(n_tick_locations)
ax_psn.set_xlabel('SPHERICAL HARMONIC DEGREE $n$', fontname = 'AppleGothic')
ax_psn.set_xlim(ax_ps.get_xlim())
ax_psn.spines['right'].set_visible(False)
ax_psn.yaxis.set_ticks_position('left')

ax_Bh.plot(Bvsh, heights, color = 'grey', zorder = 4, linewidth = 3)
...
ns = [40, 80, 200]
scalesizes = [1000, 500, 200]
labely = [70, 50, 98.5]
labels = [r'$n=40~(\sim$1000 km)', r'$n=80~(\sim$500 km)', r'$n=200~(\sim$200 km)']
angles = [63, 57, 23] # angle to rotate the label
for i, nn in enumerate(ns):
    Bvsh_smallscale = (nn * ((A + heights) / A) ** (2 * nn - 2)).sum(axis = 1)
    if SQRTPOWER:
        Bvsh_smallscale = np.sqrt(Bvsh_smallscale/Bvsh_smallscale[-1])*500
    else:
        Bvsh_smallscale = Bvsh_smallscale/Bvsh_smallscale[-1]*500

    ax_Bh.plot(Bvsh_smallscale, heights, 'k--', linewidth = .8, zorder = 1)
    labelx = np.interp(labely[i], heights.flatten(), Bvsh_smallscale)
    ax_Bh.text(labelx, labely[i], labels[i], rotation = angles[i], va = 'center', ha = 'center', bbox=dict(facecolor='white', alpha=1, edgecolor = 'white'), zorder = 3)



ax_Bh.set_xlim(0, ax_Bh.get_xlim()[1])
ax_Bh.plot(ax_Bh.get_xlim(), [85, 85], color = 'lightgray')
...
ax_Bh.text(120, 85, '85 km', va = 'center', backgroundcolor = 'white', fontname = 'AppleGothic')
ax_Bh.text(120, CURRENT_HEIGHT, str(int(CURRENT_HEIGHT)) + ' km', va = 'center', backgroundcolor = 'white', fontname = 'AppleGothic')

ax_Bh.annotate('Normalized to 500 nT at 110 km', xy=(500, 110), xytext=(500, 80), rotation = 90,
            arrowprops=dict(facecolor='gray', shrink=0.001, headwidth = 7, width = 1, edgecolor = 'gray'), 
            color = 'gray', fontname = 'AppleGothic', ha = 'center', va = 'top'
            )
ax_Bh.set_ylim(0, CURRENT_HEIGHT + 1)
ax_Bh.set_xlim(0, 500)
...
ax_Bm.set_zlim(0, CURRENT_HEIGHT)
...
plt.savefig('./figures/radial_dependence.pdf')
plt.savefig('./figures/radial_dependence.svg')
plt.savefig('./figures/radial_dependence.png', dpi = 250)
