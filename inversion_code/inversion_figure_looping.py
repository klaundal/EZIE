...
for case in ['case_3', 'case_4', 'case_5']:
    print(case)
    ...
    timestep  = td(seconds = 60) # time between each frame
    DT = info['DT']
    ...
    T0 = data.index[0] + DT//2 * td(seconds = 60)

    for counter in range( (data.index[-1] - T0) // timestep):
        tm = T0 + counter * timestep
        ...
        W = 3000#600 + RI * np.arccos(np.sum(rs[0]*rs[-1])) * 1e-3 # km
        L = 1400
        print(L, W)
        ...
        try:
            ...
            print('coulnt calculate differenation matrix. Strange. Ignoring')
            print(v, tm)
            break
        ...
        # save plots:
        plt.savefig('./figures/' + info['outputfn'] + '_loop_' + str(counter).zfill(2) + '.png', dpi = 250)
        ...
        print('saved ' + './figures/' + info['outputfn'] + '_loop_' + str(counter).zfill(2) + '.png')
