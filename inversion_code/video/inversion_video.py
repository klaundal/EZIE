...
for tc in np.arange(len(data.index)//5*3):
    ...
    ax.contourf(grid.xi, grid.eta, Gdu.dot(m).reshape(grid.eta.shape), levels = np.linspace(-700, 700, 12), cmap = plt.cm.bwr, zorder = 0, extend = 'both')
    ...
    plt.savefig('./videofigs/' + str(counter).zfill(3) + '.png', dpi = 250)
