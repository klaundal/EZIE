...
axe      = plt.subplot2grid((16, 1), (0 , 0), rowspan = 5)
axn      = plt.subplot2grid((16, 1), (5 , 0), rowspan = 5)
axr      = plt.subplot2grid((16, 1), (10 , 0), rowspan = 5)

ax_cbar = plt.subplot2grid((33, 1), (32, 0), colspan = 2)
...
cntrs = axr.contourf(grid.xi, grid.eta, np.abs(Gdu.dot(m).reshape(grid.eta.shape) - mhdBu.reshape(grid.eta.shape)), levels = np.linspace(0, 300, 12), zorder = 0, extend = 'both')

axe.contourf(grid.xi, grid.eta, np.abs(Gde.dot(m).reshape(grid.eta.shape) - mhdBe.reshape(grid.eta.shape)), levels = cntrs.levels, zorder = 0, extend = 'both')

axn.contourf(grid.xi, grid.eta, np.abs(Gdn.dot(m).reshape(grid.eta.shape) - mhdBn.reshape(grid.eta.shape)), levels = cntrs.levels, zorder = 0, extend = 'both')

ax_cbar.contourf(np.vstack((cntrs.levels, cntrs.levels)), np.vstack((np.zeros(cntrs.levels.size), np.ones(cntrs.levels.size))), np.vstack((cntrs.levels, cntrs.levels)), levels = cntrs.levels)
...
for ax in [axe, axn, axr]:
    ax.quiver(xi, eta, jxi - mhd_jxi, jeta - mhd_jeta, linewidth = 2, scale = 1e10, zorder = 40, color = 'black')#, scale = 1e10)
...
for ax, label in zip([axe, axn, axr], 
                     ['|inversion - MHD| east', '|inversion - MHD| north', '|inversion - MHD| up']):
...
plt.subplots_adjust(bottom = .05, top = .99, left = .01, right = .94)

plt.savefig('./figures/inveresion_example_difference.png', dpi = 250)
plt.savefig('./figures/inveresion_example_difference.pdf')
