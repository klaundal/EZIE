...
RI  = dict(zip(range(3), (6371.2 + np.array(HEIGHTS)) * 1e3)) # SECS height (m)
...
L = 400 + RI[1] * np.arccos(np.sum(rs[0]*rs[-1])) * 1e-3 # km
W = 1200
print(L, W)
...
for key in RI.keys():
    Ge, Gn, Gu = get_SECS_B_G_matrices(obs['lat'], obs['lon'], np.ones_like(obs['lat']) * (6371.2 + OBSHEIGHT) * 1e3, 
                                       ...
                                       current_type = 'divergence_free', RI = RI[key])
...
axr_secs = [plt.subplot2grid((16, 3), (0, i), rowspan = 15) for i in range(3)]

ax_cbar = plt.subplot2grid((33, 2), (32, 0), colspan = 3)
...
for key in RI.keys():
    ...
    B = np.sqrt(Gde.dot(m[key]).reshape(grid.eta.shape)**2 + Gdn.dot(m[key]).reshape(grid.eta.shape)**2 + Gdu.dot(m[key]).reshape(grid.eta.shape)**2)



    cntrs = axr_secs[key].contourf(grid.xi, grid.eta, Gdu.dot(m[key]).reshape(grid.eta.shape), levels = np.linspace(-700, 700, 12), cmap = plt.cm.bwr, zorder = 0, extend = 'both')
    ...
    Gje, Gjn = get_SECS_J_G_matrices(jlat, jlon, grid.lat.flatten(), grid.lon.flatten(), current_type = 'divergence_free', RI = RI[key])
    je, jn = Gje.dot(m[key]).flatten(), Gjn.dot(m[key]).flatten()
    xi, eta, jxi, jeta = projection.vector_cube_projection(je.flatten(), jn.flatten(), jlon, jlat)

    axr_secs[key].quiver(xi, eta, jxi, jeta, linewidth = 2, scale = 1e10, zorder = 40, color = 'black')#, scale = 1e10)
...
for key, ax in enumerate(axr_secs):
    label = 'Br ' + str(int(HEIGHTS[key])) + 'km'
    
    ax.text(ximin- 25/(RI[1] * 1e-3), etamax- 25/(RI[1] * 1e-3), label, va = 'top', ha = 'left', bbox = dict(facecolor='white', alpha=1), zorder = 101, size = 14)




for ax in axr_secs:
    ax.set_xlim(ximin - 25/(RI[1] * 1e-3), ximax + 25/(RI[1] * 1e-3))
    ax.set_ylim(etamin + 55/(RI[1] * 1e-3), etamax - 55/(RI[1] * 1e-3))
    ax.set_adjustable('datalim') 
    ax.set_aspect('equal')
    #ax.set_xlim(*xlim)
    #ax.set_ylim(*ylim)
...
plt.savefig('./figures/height_dependence.png', dpi = 250)
plt.savefig('./figures/height_dependence.pdf')
