...
for tc in np.arange(len(data.index)//5*3):
    # get index of central point of analysis interval:
    tm = data.index[1*len(data.index)//5:4*len(data.index)//5][tc]
    counter += 1
    ...
    ax_secs  = fig.add_subplot(122)
    axr_true = fig.add_subplot(121)
    ...
    cntrs = ax_secs.contourf(grid.xi, grid.eta, Gdu.dot(m).reshape(grid.eta.shape), levels = np.linspace(-700, 700, 12), cmap = plt.cm.bwr, zorder = 0, extend = 'both')
    ...
    ax_secs.quiver(xi, eta, jxi, jeta, linewidth = 2, scale = 1e10, zorder = 40, color = 'black')#, scale = 1e10)
    ...
    for ax in [axr_true, ax_secs]:
        ...
        ax.set_xlim(ximin - 15/(RI * 1e-3), ximax + 15/(RI * 1e-3))
        ax.set_ylim(etamin + 0/(RI * 1e-3), etamax - 0/(RI * 1e-3))
        ax.set_adjustable('datalim') 
        ax.set_aspect('equal')
        #ax.set_xlim(*xlim)
        #ax.set_ylim(*ylim)
    ...
    plt.savefig('./videofigs_mhd/' + str(counter).zfill(3) + '.png', dpi = 250)
