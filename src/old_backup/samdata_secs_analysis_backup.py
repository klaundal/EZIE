...
info = {'filename':'sam_data/EZIE_event_simulation_ezie_simulation_case_1_look_direction_case_2_retrieved_los_mag_fields.sav',
        'mapshift':-210,
        'observation_height':80,
...
data = get_data(fn = info['filename'])
...
counter = 0
for tm in data.index[1*len(data.index)//5:4*len(data.index)//5:2][20:]:
    ...
    obs = {'lat': [], 'lon': [], 'Be': [], 'Bn': [], 'Bu': []}
    for i in range(4):
        obs['lat'] += list(data.loc[t0:t1, 'lat_' + str(i + 1)].values)
        obs['lon'] += list(data.loc[t0:t1, 'lon_' + str(i + 1)].values)
        obs['Be' ] += list(data.loc[t0:t1, 'dbe_'  + str(i + 1)].values)
        obs['Bn' ] += list(data.loc[t0:t1, 'dbn_'  + str(i + 1)].values)
        obs['Bu' ] += list(data.loc[t0:t1, 'dbu_'  + str(i + 1)].values)
    ...
    GTd = G.T.dot(d)
    S = np.max(GTG)
    R = np.eye(GTG.shape[0]) * S*1e-2 + LL / np.abs(LL).max() * S * 1e1
    m = np.linalg.lstsq(GTG + R, GTd, rcond = 0)[0]
    ...
    for i in range(4):
        ...
        axmap.plot(xi, eta, color = 'C' + str(i))
    ...
    axmap.quiver(xi, eta, jxi, jeta, linewidth = 1, scale = 1e10)#, scale = 1e10)
    ...
    axmap_true.quiver(xi, eta, mhd_jxi, mhd_jeta, linewidth = 1, scale = 10, color = 'grey')#, scale = 1e10)
